...
module.exports = {
    getAllLists: async (req,res)=>{
        try{
            ...
            const sharedLists = await List.find({"invitedUsers": req.user.id}).populate("userId").lean()
    ...
    getList: async (req,res)=>{
        try{
            ...
            if (list.userId.toString() != req.user.id && !invitedUsers.filter(user => user._id == req.user.id).length > 0) {
                res.redirect('/')
            }
